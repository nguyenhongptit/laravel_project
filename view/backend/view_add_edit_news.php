...
<input type="text" name="c_name" value="<?php echo isset($arr->c_name)?$arr->c_name:""; ?>" required class="form-control">
	...
	<?php 
		//lay toan bo ban ghi trong tbl_category_news
		$category = $this->model->get_all("select * from tbl_category_news");
		foreach($category as $rows)
		{
	 ?>
<option <?php if(isset($arr->fk_category_news_id)&&$arr->fk_category_news_id==$rows->pk_category_news_id){ echo "selected"; } ?> value="<?php echo $rows->pk_category_news_id; ?>"><?php echo $rows->c_name; ?></option>
	<?php } ?>
	</select>
							</div>
						</div>
...
<?php echo isset($arr->c_content)?$arr->c_content:""; ?>								
					...
					<div class="form-group">
						<div class="row">
							...
							<div class="col-md-10">
	<input type="checkbox" <?php echo isset($arr->c_hotnews)&&$arr->c_hotnews==1 ? "checked":""; ?> name="c_hotnews"> Tin tức nổi bật
							</div>
						</div>
					</div>
					<!-- end form group -->
